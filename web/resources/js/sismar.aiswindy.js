...
Sismar.ais = function () {
    ...
    this.initialize = function (id) {
        ...
        const options = {
            key: "SREqrl19YN7zadeONKW1QURySNJ59JXD",
            verbose: false,
            lat: -23.973401,
            lon: -46.290103,
            zoom: 10
    ...
    submitFormPeriodPlayback = function (formId) {
        $('#' + formId).submit(function () {
            ...
            var startDateMilliseconds = new Date(convertDate(startDate)).getTime();
            var endDateMilliseconds = new Date(convertDate(endDate)).getTime();
    ...
    loadTypeMaps = function () {
        ...
        typeMaps = [{name: "Cidade", primary: false, layer: googleStreets},
            {name: "Hibrido", primary: false, layer: googleHybrid},
            {name: "Satélite", primary: false, layer: googleSat},
            {name: "Terreno", primary: true, layer: googleTerrain}];
    };
    ...
    getOverlayLayer = function () {
        var overlayLayer = [getNauticalChartLayer(), getInfoVesselsLayer(), getTypeVesselsLayer(), getWeatherLayer()];
        return overlayLayer;
    };
    ...
    /*getDistanceInMeters = function (p1, p2) {
     var earthRay = 6378140;
     var PI = Math.PI;
     var halfValue = 90;
     
     var v1 = Math.cos(PI * (halfValue - p2.lat) / 180);
     var v2 = Math.cos((halfValue - p1.lat) * PI / 180);
     var v3 = Math.sin((halfValue - p2.lat) * PI / 180);
     var v4 = Math.sin((halfValue - p1.lat) * PI / 180);
     var v5 = Math.cos((p1.lng - p2.lng) * PI / 180);
     
     var result = earthRay * Math.acos((v1 * v2) + (v3 * v4 * v5));
     return result;
     };*/
    ...
    fitBoundsShowAllVessels = function () {
        var latLngsBoat = [];
        for (var i = 0; i < vessels.length; i++) {
    ...
    getContentVessel = function (response) {
        ...
        var content = '<div class="container-popup-vessel">' +
                ...
                '<li>' +
                "<a class=\"button-open-period-track\" href=\"#\" class=\"ui-commandlink ui-widget\" onclick=\"eventClickOpenPeriodTrack(" + response.last_ais_record.mmsi + ");PF('open-period-track').show();PrimeFaces.ab({s:'button-open-period-track'});return false;\">" +
