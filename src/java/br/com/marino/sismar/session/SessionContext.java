package br.com.marino.sismar.session;

import br.com.marino.sismar.entity.Clientes;
import br.com.marino.sismar.entity.UsuariosWeb;
import javax.faces.context.ExternalContext;
import javax.faces.context.FacesContext;

public class SessionContext {

    private static SessionContext instance;

    public static SessionContext getInstance() {
        if (instance == null) {
            instance = new SessionContext();
        }
        return instance;
    }

    private SessionContext() {
    }

    private ExternalContext currentExternalContext() {
        if (FacesContext.getCurrentInstance() == null) {
            throw new RuntimeException("O FacesContext não pode ser chamado "
                    + "fora de uma requisição HTTP");
        } else {
            return FacesContext.getCurrentInstance().getExternalContext();
        }
    }

    public UsuariosWeb getUserLoggedIn() {
        return (UsuariosWeb) getAttribute("userLoggedIn");
    }

    public Clientes getClientLoggedIn() {
        return (Clientes) getAttribute("clientLoggedIn");
    }
    
    public void setUserLoggedIn(UsuariosWeb user, Clientes client) {
        setAttribute("userLoggedIn", user);
        setAttribute("clientLoggedIn", client);
    }

    public void closeSession() {        
        currentExternalContext().invalidateSession();
    }

    public Object getAttribute(String nome) {
        return currentExternalContext().getSessionMap().get(nome);
    }

    public void setAttribute(String nome, Object valor) {
        currentExternalContext().getSessionMap().put(nome, valor);
    }

}
